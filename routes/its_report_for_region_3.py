...
@router.get("/its_region_report_3", include_in_schema=True)
def create_region_report(
    ...
    try:
        layers = json.loads(background_layers)
        for layer in layers:
            BackgroundLayer(**layer)  # Validate each layer
    except json.JSONDecodeError:
        raise HTTPException(status_code=400, detail="Invalid JSON format for background_layers")
    except Exception as e:
            ...
            <div class="chart-container">
                <div class="chart-row">
                    ...
                    <div class="chart full-width">
                        <h3>Treatment Locations and {{ map.title }}</h3>
                        {% if map.image %}
                        ...
                        <p class="table-description" style="text-align: center;">
                            {{ map.title }} - <a href="https://caregionalresourcekits.org/clm.html" target="_blank">California Landscape Metrics</a>, 2025
                        </p>
                    </div>
                    {% endfor %}
                </div>
